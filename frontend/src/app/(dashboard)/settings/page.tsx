...
export default function SettingsPage() {
  ...
  useEffect(() => {
    api.get('/settings/tax').then((res) => {
      ...
      if (res.data.business_address) setBusinessAddress(res.data.business_address);
      if (res.data.business_phone) setBusinessPhone(res.data.business_phone);
    }).catch(() => {});
  }, []);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Business Info (for GST bills) */}
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <div className="flex items-center gap-2 mb-4">
            <Building2 size={20} className="text-gray-500" />
            <h2 className="font-semibold text-gray-900">Business Details (for GST Bills)</h2>
          </div>
          <div className="space-y-4">
            <div>
